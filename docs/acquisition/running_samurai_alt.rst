...
#. Open :code:`set_vna_params.py`
#. Change the settings for the VNA:
    * Change :code:`if_bw`, :code:`sweep_delay`, etc. to the desired values.
    * Change the :code:`visa_addr` to the comm port the VNA is on.
    * Change the :code:`mypna.setup_s_param_measurement` value to the scattering parameters the VNA will be recording (i.e :code:`[11,21,12,22]`)
#. Run :code:`set_vna_params.py`

3. Initialize the robot
++++++++++++++++++++++++
#. //FIXME MORE WORK IS NEEDED

4. Set up PNA Grabber
++++++++++++++++++++++++++++++++++++++++
...
5. Perform 2 Port VNA Calibration
++++++++++++++++++++++++++++++++++++++++
#. Make sure PNA Grabber is open from step 3
...
#. Click "measure" next to the corresponding file name on the front panel for the cal standard connected.
#. Repeat the previous 2 steps until all files have been measured.
#. Open :code:`s2p_reorganize.py` and run it to create the short, open, and load s2p files.
...
#. Make a copy of each of the `.s2p` files generated and put them into the :code:`<working-directory>/cal/calibration_pre/raw` folder

.. note:: The PNA Grabber is extraordinarily slow when recording data from a measurement. An alternate way of getting MOST (minus the switch terms) of the .s2p files is to open Spyder, running :code:`set_vna_params.py`, and then in the Spyder Console type: :code:`FIXME need code`

6. Open and update the script
++++++++++++++++++++++++++++++++++++++
