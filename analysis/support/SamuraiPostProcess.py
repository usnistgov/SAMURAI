...
class SamuraiSyntheticApertureAlgorithm:
    ...
    def load_metafile(self,metafile_path,**arg_options):
        ...
        if len(self.options['load_key']) == 1:
          ...
          for s in s_data:
            #self.all_s_parameter_data.append(np.array([s.S[load_key].raw for load_key in self.options['load_key']])) #turn the s parameters into an array
            data = np.array([s.S[load_key].raw for load_key in self.options['load_key']]) #turn the s parameters into an array
            self.all_s_parameter_data.append(data.transpose())
          self.all_s_parameter_data = np.array(self.all_s_parameter_data)
          self.freq_list = s_data[0].S[self.options['load_key'][0]].freq_list #get frequencies from first file (assume theyre all the same)
