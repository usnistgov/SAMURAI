...
numGammaVals = 10
gammaVals = np.linspace(0, np.pi, numGammaVals)
...
for iE, experiment in enumerate(expList):
    ...
    numFreqs = len(freqs)
    numMeas = measPos.shape[0]    
    
    hThetaF, music = steeringVectorsWithAntenna(measPos, freqs[[0,-1]], measS21[:,[0,-1]], angles, gammaVals, sageHorn17, mnewImp=True) 
    ...
    for iG in range(numGammaVals):
        fig = plt.figure()
        plt.imshow(np.log10(np.abs(music[1,:,iG])).reshape(uu.shape), extent=[-1.0, 1.0, -1.0, 1.0], cmap=plt.cm.jet) #plt.cm.jet)
        plt.colorbar()
        plt.xlabel('U')
        plt.ylabel('V')
        plt.title('MUSIC')
        plt.tight_layout()
        plt.savefig('music_'+experiment+'_polar_'+str(gammaVals[iG])+'_40.0.png')
        #plt.show()
